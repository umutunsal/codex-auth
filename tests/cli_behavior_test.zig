...
test "Scenario: Given unknown app subcommand when parsing then usage error is returned" {
    const gpa = std.testing.allocator;
    const args = [_][:0]const u8{ "codex-auth", "app" };
    var result = try cli.commands.parseArgs(gpa, &args);
    defer cli.commands.freeParseResult(gpa, &result);

    try expectUsageError(result, .top_level, "unknown command `app`.");
}
